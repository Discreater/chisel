...
private[plugin] trait ChiselOuterUtils {
  
  trait ChiselInnerUtils {
    def inferType(t: Tree)(using ctx: Context): Type = ctx.typer.typed(t, ctx.typer.mode).tpe
    ...
    def isNullaryMethodNamed(name: String, defdef: DefDef[Any]): Boolean =
