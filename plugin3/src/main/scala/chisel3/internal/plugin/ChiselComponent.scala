...
class ChiselComponent(arguments: ChiselPluginArguments)
    extends PluginPhase
    with ChiselOuterUtils {
  import tpd._
  val runsAfter: Set[String] = Set(TyperPhase.name)
  val phaseName: String = "chiselcomponent"
  override def apply(tree: Apply)(using ctx: Context): Tree = {
  ...
  class MyTypingTransformer(using ctx: Context) extends ChiselInnerUtils {
    ...
    def transform(tree: Tree): Tree = tree match {
      ...
      case dd @ ValDef(name, tpt, rhs) if okVal(dd) =>
