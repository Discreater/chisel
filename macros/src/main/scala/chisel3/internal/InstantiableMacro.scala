...
private[chisel3] class instantiable extends MacroAnnotation {
  override def transform(using q: Quotes) (
    ...
    //     {
    //       val extensions = mutable.ArrayBuffer.empty[Statement]
    //       // val mgVal = Symbol.newVal(Symbol.spliceOwner, "mg", TypeRepr.of[chisel3.internal.MacroGenerated])
    //       // extensions += ValDef(mgVal, Some(New(TypeTree.of[chisel3.internal.MacroGenerated], Nil)))
    //     }
    //     ???
    // }
    List(tree)
